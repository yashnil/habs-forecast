...
from align_utils import to_datetime                               # existing helper
...
# ───── calendar-reset 8-day compositor ─────────────────────────────────────
def eightday_calendar(da, how="mean"):
    """
    Group by 8-day blocks that restart every 1 Jan: DOY 1, 9, 17 … 361.

    Works on DataArray or Dataset, returns the same type.
    """
    doy     = da.time.dt.dayofyear
    offset  = ((doy - 1) % 8).astype("timedelta64[D]").data
    blk_t   = (da.time.data - offset).astype("datetime64[ns]")
    grouped = da.assign_coords(block_time=("time", blk_t)).groupby("block_time")
    fn      = grouped.mean if how == "mean" else grouped.sum
...
def build_modis() -> xr.Dataset:
    ...
    date_re  = re.compile(r"AQUA_MODIS\.(\d{8})_")
    rasters  = {}
    ...
    for var, (subdir, ncvar) in products.items():
        ...
        for fp in sorted((P_MODIS / subdir).glob("*_4km_L3m.nc")):
            ...
            if not (WIN_START <= t0 <= WIN_END):
                continue
            da = (xr.open_dataset(fp, engine="netcdf4")[ncvar]
                    .astype("float32")
                    .expand_dims(time=[t0]))
            frames.append(da)
    ...
    modis  = xr.merge([r.sel(time=common).to_dataset(name=v) for v, r in rasters.items()])
...
def build_era5(modis_time: xr.DataArray) -> xr.Dataset:
    ...
    for v,(fname,agg) in var_specs.items():
        ds = (xr.open_dataset(P_ERA5/fname,
                              engine="netcdf4",
    ...
    era8 = era8.sel(time=np.intersect1d(era8.time.values, modis_time.values))
...
def build_cmems(modis_time: xr.DataArray) -> xr.Dataset:
    ...
    parts = []
    for fp in files:
        ds = (xr.open_dataset(P_CMEMS/fp, engine="netcdf4", chunks={"time": 365})
    ...
    cm = xr.concat(parts, "time").sortby("time")
