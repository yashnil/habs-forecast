...
class ConvLSTM(nn.Module):
    """
    Channel reducer + 2-layer pixel-wise LSTM head.
    Matches the architecture in testing.py (v0.4 + PINN).
    """
    def __init__(self, Cin):
        super().__init__()
        self.reduce = nn.Conv2d(Cin, 24, 1)

        # ▶ physics coefficient (unused during inference but must be present)
        self.kappa  = nn.Parameter(torch.tensor(25., dtype=torch.float32))

        self.l1 = PxLSTM(24, 48)
        self.l2 = PxLSTM(48, 64)

        # ▶ dropout layer – parameter-free, but keep it so state_dict matches
    ...
    def forward(self, x):                     # x: (B, L, C, H, W)
        ...
        for t in range(x.size(1)):
            ...
            o2, h2 = self.l2(o1, h2)
        out = self.head(o2)
        out = self.dropout(out)               # ▶ identical to training
        return out.squeeze(1)                 # Δ-log-chl
...
def run_inference(ds, varlist, log_idx, stats, pixel_ok,
    ...
    model = ConvLSTM(len(varlist)).to(device)        # Cin already trimmed above
    model.load_state_dict(state, strict=True)        # reuse the checkpoint we read
    model.eval()
...
if __name__ == "__main__":
    ...
    # ➡ PATCH: derive Cin from checkpoint, then build a matching varlist
    state     = torch.load(args.ckpt, map_location="cpu")
    Cin_ckpt  = state["reduce.weight"].shape[1]          # e.g. 28
    varlist   = [v for v in ALL_VARS if v in ds.data_vars][:Cin_ckpt]
    if "log_chl" not in varlist:
        raise RuntimeError("log_chl missing from predictor list.")
    LOG_IDX   = varlist.index("log_chl")
    print(f"[diag] using Cin={Cin_ckpt} predictors →", ", ".join(varlist))
    # --------------------------------------------------------------------
...
subset  rmse_log_model  rmse_log_pers  mae_log_model  mae_log_pers  rmse_mg_model  rmse_mg_pers  mae_mg_model  mae_mg_pers  skill_log_pct  skill_mg_pct
 train        0.805570       0.892674       0.597141      0.617396       5.211825      6.298439      2.252473     2.622593       9.757603     17.252111
   val        0.770127       0.854255       0.569414      0.596557       4.752190      5.800528      2.036875     2.384271       9.848166     18.073154
  test        0.805165       0.888878       0.594082      0.624285       5.091966      6.138979      2.243596     2.639769       9.417910     17.055161
   all        0.799834       0.885994       0.592196      0.614930       5.123192      6.198111      2.216055     2.586010       9.724730     17.342674
'''