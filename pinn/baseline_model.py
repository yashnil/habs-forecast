...
def _LAMBDA_SCHEDULE(ep: int) -> float:
    '''
    if ep < 6:          # 1-5  → data only
        return 0.0
    elif ep < 16:       # 6-15 → 0.02, 0.04, … 0.20
        return 0.02 * (ep - 5)
    elif ep < 21:       # 16-20 hold
        return 0.20
    else:               # 21+  jump ×4  (0.80)
        return 0.80
    '''
    return min(0.10, 0.01 * max(ep-5, 0))
...
class ConvLSTM(nn.Module):
    def __init__(self,Cin):
        ...
        self.reduce=nn.Conv2d(Cin,32,1)
        self.dropout=nn.Dropout2d(0.1)
        self.l1,self.l2 = PxLSTM(32,64),PxLSTM(64,96)
...
def physics_residual(C_pred_lin, C_last_lin, u_lin, v_lin, kappa):
    ...
    # robust scaler: Median-Absolute-Deviation of *this* batch
    sigma = median(raw.abs()[raw.isfinite()]) + 1e-9
    return raw / sigma            # unit-free residual
    '''

    raw = dCdt + u_lin*dCdx + v_lin*dCdy - kappa*lap
    phys = raw / 3e-8        # ← global σ_phys
    phys = torch.clamp(phys, -10., 10.)
    return phys
...
def train_one(dl, net, opt, stats, epoch, phase2_lambda):
    net.train()
...
def main():
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        train_one(tr_dl, net, opt, stats, ep, phase2_lambda)
...
Results (V3):
FINAL RMSE_log: {'train': 0.8548787067337434, 'val': 0.8468885392666082, 'test': 0.8981118791608602}
