...
if __name__ == "__main__":
    ...
    SHIFT = -1                 # negative = backward in time

    pred_log = np.roll(pred_log, SHIFT, axis=0)   # shift model ONLY
    valid_m  = np.roll(valid_m,  SHIFT, axis=0)

    # the *last* |SHIFT| frames are now wrap-around garbage → mask them out
    valid_m[SHIFT:] = False     # when SHIFT = -1, this is valid_m[-1:]
    print(f"[diag] applied {8*SHIFT}-day backward shift (forecast aligned to earlier obs)")
    # ------------------------------------------ #
