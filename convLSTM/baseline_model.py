...
def main():
    ...
    for ep in range(1,EPOCHS+1):
        ...
        if val_rm < best-1e-3:
            best=val_rm; bad=0; torch.save(net.state_dict(),OUT_DIR/'conv_best.pt')
        else:
            bad+=1
            if bad==6: break

    net.load_state_dict(torch.load(OUT_DIR/'conv_best.pt'))
    metrics = {"train":rmse(tr_dl,net),"val":rmse(va_dl,net),"test":rmse(te_dl,net)}
    print("\nFINAL RMSE_log:",metrics)
    json.dump(metrics, open(OUT_DIR/'conv_metrics.json','w'), indent=2)
