...
for k in leads:
    ...
    y_score = pred_k[m]
    mask_ok = np.isfinite(y_score)                # <── filter out NaNs
    y_true, y_score = y_true[mask_ok], y_score[mask_ok]
    if y_true.sum() == 0 or y_true.sum() == y_true.size:
        continue  # ROC needs both classes - skip this lead if it’s all 0 or all 1
